...
import org.apache.ibatis.scripting.xmltags.XMLLanguageDriver; // <-- IMPORTANTE
...
public interface ClientMapper {
    ...
    @Lang(XMLLanguageDriver.class) // <-- solo donde hay <script>/<if>
    @Select("""
            <script>
    ...
    @Lang(XMLLanguageDriver.class) // <-- también aquí
    @Select("""
            <script>
            SELECT COUNT(*)
            FROM clients
    ...
    // Estos NO necesitan @Lang porque no usan <script>/<if>
    @Select("""
            SELECT id, first_name AS firstName, last_name AS lastName, email, phone, created_at AS createdAt
            FROM clients
            WHERE id = #{id}
